 ...
 */
extern inline guint32 hweight32(guint32 w) 
{
...
extern inline guint16 hweight16(guint16 w)
{
...
extern inline guint8 hweight8(guint8 w)
{
	w = (w & 0x55) + ((w >> 1) & 0x55);
	w = (w & 0x33) + ((w >> 2) & 0x33);
	w = (w & 0x0F) + ((w >> 4) & 0x0F);
 ...
 */
extern inline gint parity(guint32 w)
{
 ...
 */
extern inline guint32 rbits32(guint32 w)
{
...
extern inline guint16 rbits16(guint16 w)
{
...
extern inline guint8 rbits8(guint8 w)
{
	w = ((w >> 1) & 0x55) | ((w << 1) & 0xFF);
	w = ((w >> 2) & 0x33) | ((w << 2) & 0xCC);
	w = ((w >> 4) & 0x0F) | ((w << 4) & 0xF0);
...
extern inline guint8 graydecode(guint8 data)
{
	return data ^ (data >> 1);
}
 ...
 */
extern inline gdouble hamming(gdouble x)
{
 ...
 * Sinc etc...
 */
extern inline gdouble sinc(gdouble x)
{
	return (fabs(x) < 1e-10) ? 1.0 : (sin(M_PI * x) / (M_PI * x));
}

extern inline gdouble cosc(gdouble x)
{
...
extern inline gfloat clamp(gfloat x, gfloat min, gfloat max)
{
...
extern inline gfloat decayavg(gfloat average, gfloat input, gfloat weight)
{
